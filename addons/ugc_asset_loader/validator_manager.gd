...
const validator_const = preload("./base_validator.gd")
...
static func scan_ref_node_tree(p_ref_branch: RefNode, p_canvas: bool, p_validator: validator_const) -> Dictionary:
		...
		if property_value is NodePath:
			...
			if property_name == "script":
				...
				# Check if this object is a canvas
					...
					if !p_validator.is_script_valid(property_value, p_ref_branch.class_str):
						return {
...
func sanitise_packed_scene(p_packed_scene: PackedScene, p_validator: validator_const) -> Dictionary:
